        ...
        switch (requestUrl[1]) {
            ...
            case 'lff':
                self.lookForFile(requestUrl[2], function(error, data) {
                    ((error && error != 404) ?
                        self.send('custom', response, error):
                    ...
                    if (requestUrl[2] == '') {
                        ...
                        if (wellKnownPeers.indexOf('http://' + requestUrl[2]) != -1) {
                            utils.formatIp(requestUrl[2], function(errIp, ip) {
                                if (errIp) {
                                    self.send('custom', response, errIp);
                                } else {
                            ...
                            self.send('custom', response, requestUrl[2] + ' doesn\'t exists in your HashTable');
                        }
                    }
                });
                break;
...
Server.prototype.send = function(error, response, page) {
    ...
    var data = ((file != '' && error != 'custom') ? fs.readFileSync(file) : page);
