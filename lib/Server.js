...
Server.prototype.connect = function() {
    ...
    var server = http.createServer(function(request, response) {
        ...
        switch (requestUrl[1]) {
            ...
            // show notifications
            case 'nf':
                self.showNotifications(requestUrl[2], function(data) {
    ...
    utils.formatIp(ip, function(errIp, ipWithProtocol) {
        ...
        fs.readFile(config.mlFilename, function(errF, data) {
            ...
            if (!readable[ipWithProtocol]) {
                request(ipWithProtocol + ':' + self.port + '/pk', function (error, response, body) {
                    ...
                    if (!error || response.statusCode != 200) {
                        ...
                        fs.writeFile(config.mlFilename, JSON.stringify(readable), function(err) {
                            ...
                            fs.readFile(config.notifications, function(errR, noti) {
                                ...
                                if (nf['friendshipRequests']['receivedQueue'].indexOf(ipWithProtocol) != -1) {
                                    nf['friendshipRequests']['receivedQueue'].splice(nf['friendshipRequests']['receivedQueue'].indexOf(ipWithProtocol), 1);
                        ...
                        callback('Can\'t connect to the remote host', null);
                    }
                });
            } else {
                callback('This ip is already on your mirrorList', null);
            }
        });
    });
}
...
Server.prototype.showNotifications = function(url, callback) {
        ...
        if (url == '') {
            ...
            for (var ip in recQue) {
                ...
                notifications += recQue[ip] +
                    '<a href="/ah/' + recQue[ip].substr(7, recQue[ip].length) +
                    '">Accept</a><br>';
            }
        }
