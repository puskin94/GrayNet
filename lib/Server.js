...
Server.prototype.writeResponse = function(response, page) {
    if (page === 'index.html') {
        fs.readFile('../frontend/' + page, function(err,data) {
            (!err) ? response.end(data) : response.end(err);
        });
    } else if (page === 'hashTable.json') {
        fs.readFile('../jsonFiles/' + page, function(err,data) {
            (!err) ? response.end(data) : response.end(err);
        });
    } else {
        response.end(page);
...
Server.prototype.getFile = function(file, callback) {
    ...
    fs.readdir('../cache/', function(error, filename) {
        ...
        // effettuata la callback o la cartella 'cache/' è vuota, mandiamo in
        // callback un errore
        callback(404, null);
    });
}
        ...
        if (page == hash) {
            ...
            for (var address in hashTable[hash]) {
                request(hashTable[hash][address] + ':' + self.port + '/gtf/' + page, function (error, res, body) {
                    if (!error && res.statusCode == 200 && hashTable[hash][address] == hashTable[hash][hashTable[hash].length - 1]) {
                    ...
                    // controllo se è presente una copia del file in locale.
                        self.getFile(page, function(errorGf, data) {
                            if (errorGf != 404) {
                                ...
                                // if the file doesn't exists online AND locally, just warn the user
...
Server.prototype.updateIndex = function() {
    ...
    fs.readFile(config.htFilename, function(error, dataHt) {
        ...
        fs.readFile('../frontend/index.html', function(error, dataIx) {
            ...
            fs.writeFile('../frontend/index.html', res, function(err) {
                if(err) throw err;
            });
