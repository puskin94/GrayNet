...
Server.prototype.connect = function() {
    ...
    var server = http.createServer(function(request, response) {
        ...
        switch (requestUrl[1]) {
            ...
            case 'ah':
                if (requestUrl[2] == '') {
                    self.writeResponse(null, response, 'addHost');
                } else {
                    self.addWellKnownPeer(requestUrl[2], function(error, data) {
                        ((error) ? self.writeResponse(null, response, error) : self.writeResponse(null, response, data));
                    });
                }
                break;
...
Server.prototype.writeResponse = function(error, response, page) {
    ...
    var header = ((page != 'style') ? fs.readFileSync(config.header) : '');
    var data = ((file != '') ? fs.readFileSync(file) : page);
    var footer = ((page != 'style') ? fs.readFileSync(config.footer) : '');
...
Server.prototype.getRemoteContent = function(page, callback) {
    ...
    fs.readFile(config.htFilename, function(error, dataHt) {
        ...
        for (var hash in readable) {
            ...
            if (page == hash) {
                for (var address in readable[hash]) {
                    request(readable[hash][address] + ':' + self.port + '/gtf/' + page, function (error, res, body) {
                        console.log(body)
                        ...
                        } else if (readable[hash][address] == readable[hash][readable[hash].length - 1]) {
