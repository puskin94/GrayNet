...
function friendRequest(ip, callback) {
        ...
        fs.readFile(config.notifications, function(err, data) {
            ...
            hashtable.getWellKnownPeers(function(error, peers) {
                ...
                if (error || peers.indexOf(formattedIp) != -1) {
                    nf.notifications++;
                    nf['friendshipRequests']['receivedQueue'].push(formattedIp);
...
function deleteFromReceivedQueue(ip, callback) {
    fs.readFile(config.notifications, function(errR, noti) {
        ...
        if (nf['friendshipRequests']['receivedQueue'].indexOf(ip) != -1) {
            nf['friendshipRequests']['receivedQueue'].splice(nf['friendshipRequests']['receivedQueue'].indexOf(ip), 1);
            nf.notifications--;
